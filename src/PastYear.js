export default class PastAge {
  constructor(pastAge, mercuryRatio, venusRatio, marsRatio, jupiterRatio) {
    if (!pastAge || !(pastAge instanceof Date) || isNaN(pastAge.getTime())) {
      throw new Error("Invalid pastAge input. Please enter a valid Date object.");
    }
  ...
  getPastAge(planet) {
    ...
    switch (planet) {
      case "Mercury":
        return Math.round(pastAgeYears / (this.mercuryRatio * this.jupiterRatio) * 100) / 100;
      case "Venus":
        return Math.round(pastAgeYears / (this.venusRatio * this.jupiterRatio) * 100) / 100;
      case "Mars":
        return Math.round(pastAgeYears / (this.marsRatio * this.jupiterRatio) * 100) / 100;
      case "Jupiter":
